...
const Quiz = () => {
    ...
    const {
        ADAV, ADTE, ADBS, ADUI, ASNE, CLCO, DBAV, DBBS, GAME,
        HTCS, INMA, JAAV, JABS, JSPR, LAYO, MOWE, PHPP, PMAG,
        VBPR, WEBU, subjects
    ...
    useEffect(() => {
        if (questionSets[Id]) {
            const randomQuestions = questionSets[Id].sort(() => Math.random() - 0.5).slice(0, 10);
            ...
            {score === null ? (
                <>
                    {selectedProduct.length > 0 ? (
                        <>
                            <h2 className="text-2xl font-bold mb-4">Câu hỏi {currentQuestion + 1}</h2>
                            ...
                            <ul className="space-y-4">
                                {currentQuestionData.Answers.map((answer) => (
                                    <li key={answer.Id} className="flex items-center">
                                        <button
                                            onClick={() => selectAnswer(currentQuestionData.Id, answer.Id)}
                                            className={`w-full p-3 text-left rounded-lg ${userAnswers[currentQuestionData.Id] === answer.Id ? 'bg-blue-500 text-white' : 'bg-white border border-gray-300 hover:bg-gray-100'}`}
                                        >
                                            {answer.Text}
                                        </button>
                                    </li>
                ...
                <>
                    <h2 className="text-3xl font-semibold mb-6">Điểm của bạn: {score} / {selectedProduct.reduce((total, question) => total + question.Marks, 0)}</h2>
