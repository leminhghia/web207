...
const EditQuestion = () => {
    ...
    async function fetchData() {
      try {
        const res = await axios.get(
          `http://localhost:2000/api/quizzes/GetByUser`
        )
        setData(res.data)
  ...
  const handleDelete = async (quiz_id) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(
          `http://localhost:2000/api/quizzes/delete/${quiz_id}`
        )
  ...
  const filterQuizz = data.filter(
    (item) =>
      (item.title && item.title.toLowerCase().includes(search.toLowerCase())) ||
      formatDate(item.created_at).toLowerCase().includes(search.toLowerCase())
  )
            ...
            >
              ...
              <div className="flex justify-between p-4 mt-4">
                <Link
                  ...
                  className="px-2 py-2 bg-gradient-to-r from-[#4663fe] to-[#cd3dff] text-white rounded-md hover:shadow-md"
                >
                  Vào ôn thi
                </Link>
              </div>
